...
env = mo_gymnasium.make("mo-ant-2obj-v5", max_episode_steps=500)
all_rewards = []
hypervolumes = []
sparsities = []
expected_utilities = []

seeds = os.listdir("results/Ant-2d/cmorl-ipo/")

for seed in seeds:
    policies = os.listdir(f"results/Ant-2d/cmorl-ipo/{seed}/final/")
    ...
    for i in range(len(policies)):
        policy_file = torch.load(open(f"results/Ant-2d/cmorl-ipo/{seed}/final/EP_policy_{i}.pt", "rb"), weights_only=True)
        env_params = pickle.load(open(f"results/Ant-2d/cmorl-ipo/{seed}/final/EP_env_params_{i}.pkl", "rb"))
...
moppo_files = pickle.load(open("eval_results_mo-ant-2obj-v5.pkl", "rb"))
...
plt.title("Pareto Front for Ant")
# plt.grid()
plt.savefig("anthopper_rewards.png")
