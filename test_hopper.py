...
env = mo_gymnasium.make("mo-hopper-2obj-v5", max_episode_steps=500)
all_rewards = []
hypervolumes = []
sparsities = []
expected_utilities = []

seeds = os.listdir("results/Hopper-2d/cmorl-ipo/")

for seed in seeds:
    policies = os.listdir(f"results/Hopper-2d/cmorl-ipo/{seed}/final/")
    ...
    for i in range(len(policies)):
        policy_file = torch.load(open(f"results/Hopper-2d/cmorl-ipo/{seed}/final/EP_policy_{i}.pt", "rb"), weights_only=True)
        env_params = pickle.load(open(f"results/Hopper-2d/cmorl-ipo/{seed}/final/EP_env_params_{i}.pkl", "rb"))
    ...
    hypervolumes.append(hypervolume(ref_point = np.array([-100, -100]), points=front))
...
moppo_files = pickle.load(open("eval_results_mo-hopper-2obj-v5.pkl", "rb"))
front_moppo = np.array(moppo_files['pareto_front'])

plt.scatter(front[:, 0], front[:, 1], color='blue', label='C-MORL', s = 100)
plt.scatter(front_moppo[:, 0], front_moppo[:, 1], color='red', label='D3PO', s = 100)


# plt.scatter(dominated[:, 0], dominated[:, 1], color='red', label='Dominated Points')

for file in ["hopper2d_CAPQL.csv", "hopper2d_GPI-LS.csv", "hopper2d_PG-MORL.csv"]:
    data = pd.read_csv(file)
    front_file = pareto_front(data.values)
    front_file = data.values[front_file]
    print(front_file)
    plt.scatter(front_file[:, 0], front_file[:, 1], label=file[9:-4], s=100)
plt.legend()
plt.xlabel("Forward Reward")
plt.ylabel("Jump Reward")
plt.title("Pareto Front for Hopper")
# plt.grid()
plt.savefig("hopper_rewards.png")
