...
env = mo_gymnasium.make("mo-humanoid-v5", max_episode_steps=1000)
all_rewards = []
hypervolumes = []
sparsities = []
expected_utilities = []

seeds = os.listdir("results/Humanoid-2d/cmorl-ipo/")

for seed in seeds:
    policies = os.listdir(f"results/Humanoid-2d/cmorl-ipo/{seed}/final/")
    ...
    for i in range(len(policies)):
        policy_file = torch.load(open(f"results/Humanoid-2d/cmorl-ipo/{seed}/final/EP_policy_{i}.pt", "rb"), weights_only=True)
        env_params = pickle.load(open(f"results/Humanoid-2d/cmorl-ipo/{seed}/final/EP_env_params_{i}.pkl", "rb"))
...
moppo_files = pickle.load(open("eval_results_mo-humanoid-v5.pkl", "rb"))
...
for file in ["humanoid_CAPQL.csv", "humanoid_GPI-LS.csv", "humanoid_PG-MORL.csv"]:
...
plt.title("Pareto Front for Humanoid")
# plt.grid()
plt.savefig("humanoid_rewards.png")
